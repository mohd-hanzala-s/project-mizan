...
async function makeRule(overrides: Partial<RecurringRule> = {}): Promise<RecurringRule> {
...
describe('computeNextExecution (schedule math)', () => {
  it('returns the start date itself when it is on/after the anchor', () => {
    const next = computeNextExecution('2026-04-10', 'monthly', undefined, new Date(2026, 2, 15))
    expect(fmt(next)).toBe('2026-04-10')
  })

  it('daily advances by one day', () => {
    const next = computeNextExecution('2026-03-01', 'daily', undefined, new Date(2026, 2, 3))
    expect(fmt(next)).toBe('2026-03-03')
  })

  it('weekly advances in 7-day steps', () => {
    const next = computeNextExecution('2026-03-02', 'weekly', undefined, new Date(2026, 2, 15))
  ...
  it('monthly clamps day-of-month to month-end (§10 month-end transitions)', () => {
    ...
    const next = computeNextExecution('2026-01-31', 'monthly', undefined, new Date(2026, 1, 15))
    expect(fmt(next)).toBe('2026-02-28')
  })

  it('quarterly / half-yearly / yearly use month multiples', () => {
    expect(
      fmt(computeNextExecution('2026-03-01', 'quarterly', undefined, new Date(2026, 8, 15)))
    ).toBe('2026-12-01')
    expect(
      fmt(computeNextExecution('2026-03-01', 'halfYearly', undefined, new Date(2026, 7, 15)))
    ).toBe('2026-09-01')
    expect(
      fmt(computeNextExecution('2026-03-01', 'yearly', undefined, new Date(2026, 8, 15)))
    ).toBe('2027-03-01')
  })

  it('custom uses the day interval', () => {
    const next = computeNextExecution('2026-03-01', 'custom', 5, new Date(2026, 2, 12))
    expect(fmt(next)).toBe('2026-03-16')
  })

  it('a start date years in the past fast-paths instead of iterating every day', () => {
    const next = computeNextExecution('2020-01-01', 'daily', undefined, new Date(2026, 2, 15))
...
describe('RecurringService CRUD', () => {
  ...
  it('rejects a zero amount, a blank title, and a custom rule without an interval', async () => {
    ...
    await expect(makeRule({ frequency: 'custom' as RecurringFrequency })).rejects.toThrow()
    await expect(makeRule({ frequency: 'custom', customIntervalDays: 7 })).resolves.toBeDefined()
  })
  ...
  it('creates an active rule with a computed next execution', async () => {
    ...
    expect(new Date(rule.nextExecution).getTime()).toBeGreaterThanOrEqual(Date.now())
...
describe('RecurringService.generateDue', () => {
  ...
  it('creates one pending entry per missed occurrence and advances nextExecution', async () => {
    ...
    await RecurringRepository.update(rule.id, { nextExecution: iso(2026, 2, 1) })
  ...
  it('does nothing when no occurrence is due', async () => {
    const rule = await makeRule()
    await RecurringRepository.update(rule.id, { nextExecution: iso(2031, 0, 1) })
    const generated = await RecurringService.generateDue(new Date(2026, 2, 10))
    expect(generated).toHaveLength(0)
    const after = await RecurringRepository.getById(rule.id)
...
describe('RecurringService pause / resume / skipNext / remove', () => {
  ...
  it('skipNext advances past the next occurrence without generating an entry', async () => {
    ...
    const after = await RecurringRepository.getById(rule.id)
    expect(fmt(new Date(after!.nextExecution))).toBe(fmt(addOccurrence(original, 'monthly')))
  ...
  it('pause stops generation and resume re-arms from today (no backfill)', async () => {
    ...
    expect((await RecurringRepository.getAllIncludingInactive()).length).toBe(1)

    // Leave nextExecution years in the past, then resume.
    await RecurringRepository.update(rule.id, { nextExecution: iso(2020, 0, 1) })
    await RecurringService.resume(rule.id)

    const after = await RecurringRepository.getById(rule.id)
    expect(after?.active).toBe(true)
    // The years-in-the-past nextExecution is discarded, not back-filled.
    expect(new Date(after!.nextExecution).getTime()).toBeGreaterThan(Date.now())
...
describe('getRecurringAlerts', () => {
  ...
  it('surfaces an upcoming reminder within reminderDays', async () => {
    const rule = await makeRule()
    await RecurringRepository.update(rule.id, { nextExecution: iso(2026, 2, 17) })
  ...
  it('reminds on the day with reminderDays = 0 and stays silent beyond the window', async () => {
    ...
    await RecurringRepository.update(today.id, { nextExecution: iso(2026, 2, 15), reminderDays: 0 })
    const freshToday = await RecurringRepository.getById(today.id)
    expect(getRecurringAlerts([freshToday!], [], new Date(2026, 2, 15))).toHaveLength(1)

    const late = await makeRule()
    await RecurringRepository.update(late.id, { nextExecution: iso(2026, 2, 20) })
    const freshLate = await RecurringRepository.getById(late.id)
    expect(getRecurringAlerts([freshLate!], [], new Date(2026, 2, 15))).toHaveLength(0)
  })
  ...
  it('never flags paid transactions as missed', async () => {
    ...
    const alerts = getRecurringAlerts([], await allTransactions(), new Date(2026, 2, 15))
    expect(alerts).toHaveLength(0)
  })
})
...
describe('getUpcomingObligations', () => {
  ...
  it('returns the next occurrence per active rule, soonest first', async () => {
    ...
    expect(obligations[0].date.getTime()).toBeLessThan(obligations[1].date.getTime())
  ...
  it('excludes paused rules and occurrences beyond the horizon', async () => {
    ...
    await RecurringRepository.update(far.id, { nextExecution: iso(2026, 6, 1) })
