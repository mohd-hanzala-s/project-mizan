...
import { GLOBAL_BUDGET_CATEGORY_ID, type Budget, type Transaction } from '@/types/entities'
...
describe('computeBudgetStatus', () => {
  it('allocated − spent = remaining (§6)', () => {
    ...
    const transactions = [
      makeTransaction({ amount: 1200, transactionDate: new Date(2026, 2, 5).toISOString() }),
    ]
  ...
  it('a global budget sums spending across all categories', () => {
    const reference = new Date(2026, 2, 15)
    const budget = makeBudget({ categoryId: GLOBAL_BUDGET_CATEGORY_ID, monthlyLimit: 10000 })
    const transactions = [
      makeTransaction({
        categoryId: 'cat-food',
        amount: 1000,
  ...
  it('rollover adds unused budget from the previous period', () => {
    ...
    const transactions = [
      ...
      makeTransaction({ amount: 3000, transactionDate: new Date(2026, 1, 10).toISOString() }),
    ]
    const status = computeBudgetStatus(budget, transactions, 1, reference)
  ...
  it('rollover can go negative if the previous period was overspent', () => {
    ...
    const transactions = [
      ...
      makeTransaction({ amount: 6000, transactionDate: new Date(2026, 1, 10).toISOString() }),
    ]
    const status = computeBudgetStatus(budget, transactions, 1, reference)
  ...
  it('severity bands: ok / warning / over', () => {
    ...
    const warningBudget = makeBudget({ monthlyLimit: 1000, warningThreshold: 80 })
    const overBudget = makeBudget({ monthlyLimit: 1000, warningThreshold: 80 })

    const ok = computeBudgetStatus(
      okBudget,
      [makeTransaction({ amount: 500, transactionDate: reference.toISOString() })],
      1,
      reference
    )
    const warning = computeBudgetStatus(
      warningBudget,
      [makeTransaction({ amount: 850, transactionDate: reference.toISOString() })],
      1,
      reference
    )
    const over = computeBudgetStatus(
      overBudget,
      [makeTransaction({ amount: 1200, transactionDate: reference.toISOString() })],
      1,
      reference
    )
  ...
  it('forecast projects current pace across the full period', () => {
    ...
    const budget = makeBudget({ monthlyLimit: 5000 })
    const transactions = [
      makeTransaction({ amount: 1000, transactionDate: new Date(2026, 1, 5).toISOString() }),
    ]
    const status = computeBudgetStatus(budget, transactions, 1, reference)
...
describe('BudgetService CRUD', () => {
  ...
  it('deactivate removes a budget from getAll()', async () => {
    ...
    const { BudgetRepository } = await import('@/repositories/BudgetRepository')
