...
describe("getRecentTransactions", () => {
  it("excludes soft-deleted and respects the limit, newest first", () => {
    const transactions = [
      makeTransaction({
        id: "1",
        transactionDate: new Date(2026, 0, 1).toISOString(),
      }),
      makeTransaction({
        id: "2",
        transactionDate: new Date(2026, 0, 3).toISOString(),
      }),
      makeTransaction({
        id: "3",
...
describe("getAlerts", () => {
  it("flags a negative-balance non-credit-card account", () => {
    const accounts = [
      makeAccount({ id: "a1", type: "cash", currentBalance: -50 }),
    ];
  ...
  it("does not flag a negative credit card balance (expected/normal)", () => {
    const accounts = [
      makeAccount({ id: "a1", type: "creditCard", currentBalance: -500 }),
    ];
