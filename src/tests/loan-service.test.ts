...
describe("firstDueDate", () => {
  it("uses the due day on/after the start date", () => {
    expect(
      fmt(firstDueDate(makeLoan({ startDate: "2026-01-15", dueDay: 20 }))),
    ).toBe("2026-01-20");
  });

  it("rolls into the next month when the start date has passed this month's due day", () => {
    expect(
      fmt(firstDueDate(makeLoan({ startDate: "2026-01-25", dueDay: 20 }))),
    ).toBe("2026-02-20");
  });

  it("clamps day 31 to the end of short months (§10 month-end transitions)", () => {
    expect(
      fmt(firstDueDate(makeLoan({ startDate: "2026-02-01", dueDay: 31 }))),
    ).toBe("2026-02-28");
  });
});
...
describe("nextDueDate", () => {
  it("returns the first due date when no EMI is due yet", () => {
    ...
    expect(fmt(nextDueDate(loan, new Date(2026, 0, 10)) as Date)).toBe(
      "2026-01-20",
    );
  });
  ...
  it("advances strictly past the reference date", () => {
    ...
    expect(fmt(nextDueDate(loan, new Date(2026, 5, 20)) as Date)).toBe(
      "2026-07-15",
    );
  });
  ...
  it("returns null once the loan is paid off", () => {
    expect(
      nextDueDate(makeLoan({ currentBalance: 0 }), new Date(2026, 5, 20)),
    ).toBeNull();
  });
});
...
describe("splitPayment", () => {
  it("full payoff goes entirely to principal", () => {
    const split = splitPayment(
      makeLoan({ currentBalance: 10000, interestRate: 12 }),
      10000,
    );
  ...
  it("charges interest on the balance first when interest is tracked", () => {
    ...
    expect(splitPayment(loan, 1000)).toEqual({
      principalPaid: 900,
      interestPaid: 100,
    });
  });
  ...
  it("a payment below the month's interest reduces no principal", () => {
    const loan = makeLoan({ currentBalance: 10000, interestRate: 12 });
    expect(splitPayment(loan, 50)).toEqual({
      principalPaid: 0,
      interestPaid: 50,
    });
  });
...
describe("getPayoffForecast", () => {
  ...
  it("returns zero remaining for a paid-off loan", () => {
    const forecast = getPayoffForecast(
      makeLoan({ currentBalance: 0, status: "completed" }),
    );
...
describe("LoanService CRUD", () => {
  ...
  it("completes the loan when the balance reaches zero and blocks further payments", async () => {
    const loan = await createLoan(
      makeLoan({ currentBalance: 1000, monthlyEMI: 1000 }),
    );
    ...
    await expect(
      LoanService.recordPayment(loan.id, {
  ...
  it("delete removes the loan and its payment history together", async () => {
    ...
    expect(await db.loan_payments.where("loanId").equals(loan.id).count()).toBe(
      0,
    );
  });
});
...
describe("LoanService.getAlerts", () => {
  ...
  it("alerts when the EMI is due tomorrow", () => {
    ...
    expect(alerts.some((a) => a.id === `loan-due-tomorrow-${loan.id}`)).toBe(
      true,
    );
    expect(
      alerts.find((a) => a.id === `loan-due-tomorrow-${loan.id}`)?.severity,
    ).toBe("info");
  });
  ...
  it("alerts when the loan is overdue", () => {
    ...
    expect(
      alerts.find((a) => a.id === `loan-overdue-${loan.id}`)?.severity,
    ).toBe("warning");
  });
  ...
  it("alerts when a recently completed loan is paid off", async () => {
    ...
    expect(
      alerts.find((a) => a.id === `loan-completed-${loan.id}`),
    ).toBeDefined();
  });

  it("alerts on a recent extra payment", async () => {
    const loan = await createLoan(
      makeLoan({ currentBalance: 5000, monthlyEMI: 1000 }),
    );
    const payment = await LoanService.recordPayment(loan.id, {
      paymentDate: "2026-08-01",
      amountPaid: 1200,
    ...
    expect(
      alerts.find((a) => a.id === `loan-extra-payment-${loan.id}`)?.message,
    ).toContain("200");
  });
  ...
  it("drops informational alerts once they are older than 30 days", async () => {
    ...
    expect(alerts.some((a) => a.id === `loan-extra-payment-${loan.id}`)).toBe(
      false,
    );
  });
});
