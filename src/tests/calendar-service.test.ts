...
function payments(): LoanPayment[] {
  return [
    makePayment(homeLoan.id, { paymentDate: "2026-06-10", amountPaid: 10000 }),
  ];
}
...
describe("getMonthEvents", () => {
  ...
  it("includes future recurring occurrences only, for active rules, within the month", () => {
    ...
    expect(gym.some((d) => d.getTime() === iso(2026, 5, 19).getTime())).toBe(
      true,
    );
    expect(gym.some((d) => d.getTime() === iso(2026, 5, 26).getTime())).toBe(
      true,
    );
...
describe("day/week helpers", () => {
  ...
  it("getWeekEvents returns only events in the 7-day window", () => {
    ...
    for (const e of week) {
      ...
      expect(e.date.getTime()).toBeLessThan(
...
describe("filterEvents", () => {
  ...
  it("filters by kind", () => {
    ...
    expect(
      filterEvents(events, "", ["transaction", "recurring"]).map((e) => e.id),
    ).toEqual(["t1", "r1"]);
  });
