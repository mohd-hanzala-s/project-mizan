...
export function SmartEntryInput({
  ...
  const [rawText, setRawText] = useState(
    initial
      ? `${initial.amount ?? ""} ${initial.description ?? ""}`.trim()
      : "",
  );
  const [manualType, setManualType] = useState<"expense" | "income" | null>(
    initial?.type ?? null,
  );
  ...
  const canSave = Boolean(
    parsed.amount && parsed.amount > 0 && categoryId && accountId,
  );
  ...
  async function handleSave() {
    ...
    try {
      const input: CreateTransactionInput = {
        amount: parsed.amount,
        description:
          parsed.description || (type === "income" ? "Income" : "Expense"),
        type,
        categoryId,
        accountId,
        transactionDate: new Date().toISOString(),
      ...
      setError(
        e instanceof Error ? e.message : "Could not save this transaction.",
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="flex flex-col gap-24">
        ...
        <div className="flex items-center justify-between">
          ...
          {activeSuggestion &&
            ...
            activeSuggestion.confidence < AUTO_ASSIGN_THRESHOLD && (
              <span className="text-caption text-income">
      ...
      {activeDuplicateWarning && (
        <div className="flex items-start gap-8 rounded-md border border-warning/40 bg-warning-subtle p-12 text-body-sm text-text-primary">
          <AlertTriangle
            className="mt-4 size-16 shrink-0 text-warning"
            aria-hidden="true"
          />
