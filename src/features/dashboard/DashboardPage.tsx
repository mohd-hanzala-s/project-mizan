...
export function DashboardPage() {
  ...
  const timeline = useMemo(
    () => getSpendingTimeline(transactions, 7),
    [transactions],
  );
  ...
  const obligations = useMemo(
    () => getUpcomingObligations(recurringRules),
    [recurringRules],
  );
        ...
        {obligations.length === 0 ? (
          ...
          <div className="flex flex-col divide-y divide-border-subtle overflow-hidden rounded-md border border-border">
            {obligations.slice(0, 5).map((o) => (
              <div
                key={o.ruleId}
                className="flex items-center justify-between gap-8 px-16 py-12"
              >
      ...
      >
        {loans.length === 0 ? (
          <p className="flex items-center gap-8 text-body-sm text-text-secondary">
            ...
            Track home, car, and personal loans — add one to start tracking your
            EMI payoff.
          </p>
        ) : (
          <div className="flex flex-col divide-y divide-border-subtle overflow-hidden rounded-md border border-border">
