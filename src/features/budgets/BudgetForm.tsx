...
export function BudgetForm({ editing, onSaved, onCancel }: BudgetFormProps) {
  ...
  const [budgetedCategoryIds, setBudgetedCategoryIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const [monthlyLimit, setMonthlyLimit] = useState<number | null>(
    editing?.monthlyLimit ?? null,
  );
  const [rolloverEnabled, setRolloverEnabled] = useState(
    editing?.rolloverEnabled ?? false,
  );
  const [warningThreshold, setWarningThreshold] = useState(
    editing?.warningThreshold ?? 80,
  );
  ...
  useEffect(() => {
    db.categories
      .toArray()
      .then((all) => setCategories(all.filter((c) => !c.isArchived)));
    ...
    <div className="flex flex-col gap-24">
      ...
      <label className="flex min-h-touch items-center justify-between rounded-md border border-border bg-surface-card px-16">
        <span className="text-body-sm text-text-primary">
          Roll over unused budget next month
        </span>
        <input
          type="checkbox"
