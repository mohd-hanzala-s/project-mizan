...
export function getCurrentPeriod(
  ...
  const anchorMonth =
    day >= budgetMonthStart ? reference.getMonth() : reference.getMonth() - 1;
...
function sumByType(
  ...
  return transactions
    .filter(
      (t) =>
        !t.isDeleted && t.type === type && inRange(t.transactionDate, range),
    )
...
export function getSpendingTimeline(
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    buckets.push({
      date,
      total: sumByType(transactions, "expense", { start: date, end: next }),
    });
  }
  return buckets;
}
...
export function getAlerts(accounts: Account[]): DashboardAlert[] {
  return accounts
    .filter(
      (a) => !a.isArchived && a.type !== "creditCard" && a.currentBalance < 0,
    )
    .map((a) => ({
