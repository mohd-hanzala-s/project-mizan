...
for (const file of walk("src")) {
  ...
  for (const match of content.matchAll(spacingPattern)) {
    ...
    if (!ALLOWED_SPACING.has(value)) {
      problems.push(
        `${file}: "${match[0]}" — ${value} isn't in the allowed spacing scale`,
      );
    }
  }
  ...
  for (const match of content.matchAll(radiusPattern)) {
    ...
    if (
      size === null ||
      (!ALLOWED_RADIUS.has(size) &&
        !/^(t|b|l|r|tl|tr|bl|br|s|e|ss|se|es|ee)$/.test(size))
    ) {
      problems.push(
  ...
  for (const match of content.matchAll(palettePattern)) {
    problems.push(
      `${file}: "${match[0]}" — raw Tailwind color, use a semantic token instead`,
    );
  }
}
...
if (problems.length > 0) {
  ...
  console.error(
    "color system (palette) — see tailwind.config.js and src/theme/tokens.css.",
  );
  process.exit(1);
} else {
  console.log(
    "Design token audit passed — no out-of-scale or raw-palette classes found.",
  );
}
